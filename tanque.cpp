...
unsigned int Tanque::capacidadInicial(Partida partidaActual) {

	int dificultad = partidaActual.obtenerDificultad();
	unsigned int capacidad ;

	if(dificultad == 1){
		//HAY QUE DEFINIR LA FORMULA
		capacidad = 25 ; //pongo este valor para compilar
	}
	if(dificultad == 2){
		//HAY QUE DEFINIR LA FORMULA
		capacidad = 25 ; //pongo este valor para compilar
